...
class SportsCardApp:
    ...
    def __init__(self, master):
        ...
        # Setting up database
        self.conn = sqlite3.connect("sports_cards.db")
    ...
    def create_widgets(self):
        ...
        labels = ["Player:", "Year:", "Brand:", "Card Number:", "Value:"]
        self.entries = {}
    ...
    def add_card(self):
        ...
        try:
            ...
            self.cursor.execute("INSERT INTO cards VALUES (NULL, ?, ?, ?, ?, ?, ?)", 
                               (player, year, brand, card_number, value))
            self.conn.commit()
            self.view_all_cards()
            self.clear_entries()
    ...
    def update_card(self):
        ...
        try:
            ...
            self.cursor.execute("""
                UPDATE cards SET player=?, year=?, brand=?, card_number=?, value=? WHERE id=?
            """, (player, year, brand, card_number, value, card_id))
            self.conn.commit()
            self.view_all_cards()
            self.clear_entries()
        except (ValueError, sqlite3.Error) as e:
